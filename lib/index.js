var Index = (() => {
    ...
    function addEntry(filepath, filename, physicalpath) {
        if (physicalpath == undefined) physicalpath = filepath + '/' + filename;
        ...
        fileNode.addEventListener('click', ({ target }) => ANN.initAnn(target.dataset.path));
...
if (params.get('file') != undefined) ANN.initAnn(params.get('file'));
if (params.get('event') != undefined) ANN.changeEvent(params.get('event'));
